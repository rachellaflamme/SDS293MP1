...
##fitting training data to model with df=10.5
fitted_spline_model4 <- smooth.spline(x = trainingcv_4$log_liv, y = trainingcv_4$SalePrice, df = 10.5)

##converting output into a dataframe
fitted_spline_model_points4 <- fitted_spline_model4 %>%
  broom::augment()
fitted_spline_model_points4

#graphing training results
ggplot(data = trainingcv_4, aes(x = log_liv, y = SalePrice)) +
  geom_point() + geom_line(data = fitted_spline_model_points4, aes(x = x, y = .fitted), col = "blue", size = 1)

##using test data with model, df = 10.5
fitted_spline_model_test4 <- smooth.spline(x = testcv_4$log_liv, y = testcv_4$SalePrice, df = 10.5)

fitted_spline_model_points_test4 <- fitted_spline_model_test4 %>%
  broom::augment()
fitted_spline_model_points_test4

##calculate RMSE of test data
rmse_frame4 <- fitted_spline_model_points_test4%>%
  mutate(
    squared_residual = .resid^2
    ) %>%
  summarize(mse = mean(squared_residual)) %>%
...
ggplot(data = testcv_4, aes(x = log_liv, y = SalePrice)) +
  geom_point() + geom_line(data = fitted_spline_model_points_test4, aes(x = x, y = .fitted), col = "blue", size = 1)
```
```{r}
...
##fitting training data to model with df=9.75
fitted_spline_model5 <- smooth.spline(x = trainingcv_5$log_liv, y = trainingcv_5$SalePrice, df = 9.75)

##converting output into a dataframe
fitted_spline_model_points5 <- fitted_spline_model5 %>%
  broom::augment()
fitted_spline_model_points5

#graphing training results
ggplot(data = trainingcv_5, aes(x = log_liv, y = SalePrice)) +
  geom_point() + geom_line(data = fitted_spline_model_points5, aes(x = x, y = .fitted), col = "blue", size = 1)

##using test data with model, df = 9.75
fitted_spline_model_test5 <- smooth.spline(x = testcv_5$log_liv, y = testcv_5$SalePrice, df = 9.75)

fitted_spline_model_points_test5 <- fitted_spline_model_test5 %>%
  broom::augment()
fitted_spline_model_points_test5

##calculate RMSE of test data
rmse_frame5 <- fitted_spline_model_points_test5%>%
  mutate(
    squared_residual = .resid^2
    ) %>%
  summarize(mse = mean(squared_residual)) %>%
...
ggplot(data = testcv_5, aes(x = log_liv, y = SalePrice)) +
  geom_point() + geom_line(data = fitted_spline_model_points_test5, aes(x = x, y = .fitted), col = "blue", size = 1)
```
